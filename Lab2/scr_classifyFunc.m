function [ score ] = scr_classifyFunc(props)
% This function classify the textures of the dataset. 
...
for i=1:length(d),
	...
	for j=1:2,
		...
		A = imread(name);
		vecTrain((i-1)*2+j,:) = computeFeatureVector(A, props); %training vector
		labTrain((i-1)*2+j) = i+48; %training labels
	end
	for j=3:length(d1),
		name = [dataDir namedir '/' d1(j).name];
		A = imread(name);
		vecTest((i-1)*4+j-2,:) = computeFeatureVector(A, props); %testing vector
